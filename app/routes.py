    ...
    for manifest in CharterManifest.query.filter_by(closed_at=None).all():
        try:
            ...
            for passenger in passengers:
                ...
                dep, ades = html.escape((manifest.dep or "---").upper()), html.escape((manifest.ades or "---").upper())
                body = f'''<!doctype html><html><body style="margin:0;padding:0;background:#f1f2ef;color:#15333d;font-family:Arial,Helvetica,sans-serif"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#f1f2ef"><tr><td align="center" style="padding:34px 14px"><table role="presentation" width="620" cellspacing="0" cellpadding="0" border="0" style="width:100%;max-width:620px;background:#ffffff"><tr><td style="height:7px;background:#b89552;font-size:0;line-height:0">&nbsp;</td></tr><tr><td style="padding:30px 38px 25px;background:#073f51"><img src="{html.escape(logo_url, quote=True)}" width="172" alt="ACCharters" style="display:block;width:172px;max-width:100%;height:auto;background:#fff;padding:5px 9px"><p style="margin:32px 0 8px;color:#d8c391;font-size:11px;line-height:14px;letter-spacing:2px;font-weight:bold">YOUR CHARTER JOURNEY</p><h1 style="margin:0;color:#fff;font-size:32px;line-height:38px;font-weight:normal">Pre-check-in is ready</h1></td></tr><tr><td style="padding:32px 38px 12px"><p style="margin:0 0 20px;font-size:17px;line-height:26px">We look forward to welcoming you on board. Select your preferred seat and let us know if you are travelling with checked baggage.</p><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border:1px solid #d8e0dc;background:#f7faf8"><tr><td style="padding:18px 20px"><p style="margin:0 0 6px;color:#6a7c7d;font-size:10px;letter-spacing:1.5px;font-weight:bold">YOUR FLIGHT</p><p style="margin:0;color:#073f51;font-size:25px;line-height:31px;font-weight:bold">{dep} <span style="color:#b89552">&#8594;</span> {ades}</p><p style="margin:10px 0 0;color:#425c62;font-size:14px;line-height:20px"><strong>{html.escape(manifest.flight_no or 'CHARTER')}</strong>&nbsp;&nbsp;|&nbsp;&nbsp;{html.escape(departure_label)}</p></td></tr></table><p style="margin:24px 0 0;font-size:14px;line-height:22px"><strong>Carry-on allowance:</strong> one bag up to 7 kg.</p></td></tr><tr><td align="center" style="padding:24px 38px 34px"><table role="presentation" cellspacing="0" cellpadding="0" border="0"><tr><td bgcolor="#b89552" style="border-radius:2px"><a href="{html.escape(link, quote=True)}" style="display:inline-block;padding:15px 28px;color:#fff;font-size:15px;font-weight:bold;text-decoration:none;letter-spacing:.2px">COMPLETE PRE-CHECK-IN</a></td></tr></table><p style="margin:20px 0 0;color:#6a7c7d;font-size:12px;line-height:18px">Your checked baggage will be weighed by our check-in team at the airport.</p></td></tr><tr><td style="padding:19px 38px;background:#edf1ef;color:#617276;font-size:11px;line-height:16px">ACCharters · An Air Chathams Service<br>This link is personal to you and valid for this charter sector.</td></tr></table></td></tr></table></body></html>'''
...
def _send_charter_boarding_pass_email(manifest: CharterManifest, passenger: dict) -> bool:
    ...
    bag_notice_html = '<div style="margin:0 0 18px;padding:13px 15px;border-left:4px solid #b89552;background:#fbf7ed;color:#4a3a1c;font-size:14px;line-height:20px"><strong>Checked baggage:</strong> Please see a check-in agent to weigh and accept your bag before proceeding to the gate lounge.</div>' if passenger.get("BagToWeigh") else ""
    bag_notice_text = "\n\nChecked baggage: Please see a check-in agent to weigh and accept your bag before proceeding to the gate lounge." if passenger.get("BagToWeigh") else ""
    ...
    html_body = f'''<div style="max-width:640px;margin:auto;padding:24px 14px;font-family:Arial,sans-serif;color:#173743;background:#e8f5f3"><div style="overflow:hidden;border-radius:18px;background:#fff;box-shadow:0 10px 28px #073b5c24"><div style="padding:22px 26px 20px;background:linear-gradient(135deg,#075c74,#148596);color:#fff"><img src="{html.escape(logo_url, quote=True)}" alt="ACCharters" style="display:block;max-width:180px;height:auto;background:#fff;border-radius:7px;padding:4px 9px"><p style="margin:20px 0 0;font-size:11px;letter-spacing:1.5px;font-weight:bold">CHARTER BOARDING PASS</p><h1 style="margin:7px 0 0;font-size:30px;line-height:1.12">{dep} &rarr; {ades}</h1></div><div style="padding:24px 26px"><p style="margin:0 0 18px">Hello {name}, you are checked in. Please keep this pass handy for boarding.</p>{bag_notice_html}<table role="presentation" style="width:100%;border-collapse:separate;border-spacing:0 10px;font-size:14px"><tr><td style="width:42%;color:#5c7478;font-weight:bold">Departure</td><td style="font-weight:bold">{html.escape(departure)}</td></tr><tr><td style="color:#5c7478;font-weight:bold">Flight</td><td style="font-weight:bold">{html.escape(flight_no)}</td></tr><tr><td style="color:#5c7478;font-weight:bold">Aircraft</td><td style="font-weight:bold">{html.escape(aircraft)}</td></tr><tr><td style="color:#5c7478;font-weight:bold">Seat · Gate</td><td style="font-weight:bold">{html.escape(str(passenger.get('Seat') or 'GATE'))} · {html.escape(gate)}</td></tr></table><p style="margin:22px 0 16px;text-align:center"><img src="{html.escape(qr_url, quote=True)}" alt="Boarding QR code" width="170" height="170" style="display:block;margin:auto;padding:8px;border:1px solid #c9dfe0;border-radius:10px"></p><div style="text-align:center"><a href="{html.escape(apple_url, quote=True)}" style="display:inline-block;margin:6px;padding:12px 17px;background:#000;color:#fff;border-radius:7px;text-decoration:none;font-weight:bold">Add to Apple Wallet</a>{google_button}</div><p style="margin:18px 0 0;font-size:12px;color:#5c7478">Present the QR code at boarding. This pass is valid only for this charter sector.</p></div></div></div>'''
    text_body = f"ACCharters boarding pass\n\n{dep} - {ades}\nDeparture: {departure}\nPassenger: {name}\nFlight: {flight_no}\nAircraft: {aircraft}\nSeat: {passenger.get('Seat') or 'GATE'}\nGate: {gate}{bag_notice_text}\n\nApple Wallet: {apple_url}\nGoogle Wallet: {google_url or 'Unavailable'}"
...
def api_charter_self_checkin(token: str):
    ...
    was_booked = _normalise_charter_status(passenger.get("Status")) == "Booked"
    ...
    if was_booked and wallet_passenger.get("Email"):
        try:
            boarding_pass_email_sent = _send_charter_boarding_pass_email(saved_manifest, wallet_passenger)
        except Exception:
            current_app.logger.exception("Unable to email charter boarding pass after self check-in for %s", wallet_passenger.get("PassengerId"))
    return jsonify(ok=True, message="Your pre-check-in has been saved.", boarding_pass_email_sent=boarding_pass_email_sent, wallet={"apple_url": apple_url, "google_url": google_url})
...
def _resolve_kmh_crew_setup_context(
    token: str,
    flight_type_id: int,
    model_id: int,
    reg_id: int,
    ...
    """
    Return the best matching crew setup and its item list for the supplied flight context.
    KMH crew creation needs the same setup shape Envision uses for change-type flows.
    """
    setups = envision_get_crew_position_setups(token)
    items = envision_get_crew_position_setup_items(token)
    ...
    matching = [
        s for s in (setups if isinstance(setups, list) else [])
        if int(s.get("journeyTypeId") or 0) == int(flight_type_id)
        and int(s.get("modelId") or 0) == int(model_id)
    ]
    exact_setup = next((s for s in matching if int(s.get("regId") or 0) == reg_id and reg_id > 0), None)
    fallback_setup = next((s for s in matching if int(s.get("regId") or 0) == 0), None)
    chosen_setup = exact_setup or fallback_setup
...
def _find_kmh_setup_item_for_position(setup_context: dict | None, position_id: int) -> dict | None:
    if not setup_context:
        return None
    for item in setup_context.get("items") or []:
        try:
            if int(item.get("crewPositionId") or 0) == int(position_id):
                return item
        except (TypeError, ValueError):
            continue
    return None
