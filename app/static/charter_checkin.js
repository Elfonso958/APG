(() => {
  ...
  function renderPassengers() {
    ...
    shown.filter((p) => !["Boarded", "Flown"].includes(statusOf(p))).forEach((p) => { const actions = els.rows.querySelector(`tr[data-id="${CSS.escape(String(p.PassengerId))}"] .row-actions`); if (actions) actions.insertAdjacentHTML("beforeend", `<button class="button danger" data-remove-passenger="${esc(p.PassengerId)}">Remove</button>`); });
  }
