(() => {
  ...
  function renderPassengers() {
    ...
    els.rows.innerHTML = shown.length ? shown.map((p) => {
      ...
      const confirmDangerousGoods = dangerousGoodsPending ? `<button class="button" data-action="confirm-dangerous-goods" data-id="${id}">Confirm DG spoken to</button>` : "";
      const board = status === "Checked In" && !dangerousGoodsPending ? `<button class="button" data-action="board" data-id="${id}">Board</button>` : "";
